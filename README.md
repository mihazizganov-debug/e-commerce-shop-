...
#### 10. **Обработка исключений (новое в 17.2)**:
   - **Задание 1**: При создании товара с нулевым количеством выбрасывается `ValueError` с сообщением "Товар с нулевым количеством не может быть добавлен"
   - **Задание 2**: В классе `Category` реализован метод `middle_price()` для подсчета среднего ценника
   - **Обработка ошибок**: В методе `middle_price()` обработан случай пустой категории через `try/except` - возвращается 0
   - **Исключения**: Использованы `raise` для прерывания программы и `try/except` для graceful handling
